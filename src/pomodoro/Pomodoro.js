...
function nextSession(focusTimer, breakTimer) {
  /**
...
function Pomodoro() {
  ...
  const handleFocusDecrease = () => {
    setFocusTimer(Math.max(5, focusTimer - 5));
    setSession({
      label: "Focus",
      timeRemaining: (focusTimer - 5) * 60,
    });
  };
  const handleFocusIncrease = () => {
    setFocusTimer(Math.min(60, focusTimer + 5));
    setSession({
      label: "",
      timeRemaining: (focusTimer + 5) * 60,
    });
  ...
  const handleBreakDecrease = () => {
    setBreakTimer(Math.max(1, breakTimer - 1));
  };
  const handleBreakIncrease = () => {
    setBreakTimer(Math.min(15, breakTimer + 1));
  };

  function getHeading() {
    if (currentMode === "focus") {
      return `Focusing for ${minutesToDuration(focusTimer)} minutes`;
    }
    if (currentMode === "break") {
      return `On Break for ${secondsToDuration(breakTimer)} minutes`;
    }
  }
  ...
  //   setDisabledBtn(
  //     isTimerRunning ? disabledBtn === true : disabledBtn === false
  //   );
  // };
  /**
